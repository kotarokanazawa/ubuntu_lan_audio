...
class AudioBridge:
    def __init__(self, config: AudioConfig, logger: Optional[Callable[[str], None]] = None):
        ...
        self._vis_samples = max(self.cfg.samplerate // 2, self.frame_samples * 16)
        self._vis_input = np.zeros(self._vis_samples, dtype=np.float32)
        self._vis_rx = np.zeros(self._vis_samples, dtype=np.float32)
        self._last_input_rms = 0.0
        self._last_rx_rms = 0.0
        self._last_voice_detected = False
        self._last_audio_tx_at = 0.0
        self._last_audio_rx_at = 0.0

    def _append_visual(self, which: str, pcm_i16: np.ndarray) -> None:
        pcm = pcm_i16.astype(np.float32) / 32768.0
        n = min(len(pcm), self._vis_samples)
        with self._vis_lock:
            if which == "input":
                self._vis_input = np.roll(self._vis_input, -n)
                self._vis_input[-n:] = pcm[-n:]
                self._last_input_rms = float(np.sqrt(np.mean(pcm * pcm) + 1e-12))
            else:
                self._vis_rx = np.roll(self._vis_rx, -n)
                self._vis_rx[-n:] = pcm[-n:]
                self._last_rx_rms = float(np.sqrt(np.mean(pcm * pcm) + 1e-12))

    def get_visual_state(self) -> Dict[str, Any]:
        with self._vis_lock:
            return {
                "input_wave": self._vis_input.copy(),
                "rx_wave": self._vis_rx.copy(),
                "input_rms": self._last_input_rms,
                "rx_rms": self._last_rx_rms,
                "voice": self._last_voice_detected,
                "last_audio_tx_at": self._last_audio_tx_at,
                "last_audio_rx_at": self._last_audio_rx_at,
    ...
    def _on_input(self, indata, frames, time_info, status) -> None:
        ...
        with self._vis_lock:
            self._last_voice_detected = bool(voice)
        ...
        try:
            ...
            with self._vis_lock:
                self._last_audio_tx_at = time.time()
        except OSError as e:
    ...
    def _recv_loop(self) -> None:
        while self.running:
            ...
            self._append_visual("rx", decoded)
            with self._vis_lock:
                self._last_audio_rx_at = time.time()
...
def run_gui(default_args: argparse.Namespace) -> None:
    ...
    class ScopeCanvas(tk.Canvas):
        def __init__(self, master, title: str, kind: str = "wave", **kwargs):
            super().__init__(master, highlightthickness=0, bg="#111827", **kwargs)
            self.title = title
            self.kind = kind
            self.bind("<Configure>", lambda _e: self.redraw(np.zeros(256, dtype=np.float32), 16000))

        def redraw(self, samples: np.ndarray, samplerate: int) -> None:
            self.delete("all")
            w = max(10, int(self.winfo_width()))
            h = max(10, int(self.winfo_height()))
            self.create_rectangle(0, 0, w, h, fill="#111827", outline="")
            self.create_text(10, 10, anchor="nw", text=self.title, fill="#e5e7eb", font=("TkDefaultFont", 9, "bold"))
            self.create_text(w - 10, 10, anchor="ne", text=self.kind.upper(), fill="#94a3b8", font=("TkDefaultFont", 8))
            plot_top = 24
            plot_h = h - plot_top - 8
            if plot_h < 20:
                return
            self.create_line(0, plot_top + plot_h / 2, w, plot_top + plot_h / 2, fill="#334155")
            for frac in (0.25, 0.5, 0.75):
                y = plot_top + plot_h * frac
                self.create_line(0, y, w, y, fill="#1f2937")
            ...
            if self.kind == "wave":
                if data.size > w:
                    idx = np.linspace(0, data.size - 1, w).astype(np.int32)
                    data = data[idx]
                x_coords = np.linspace(0, w - 1, data.size)
                y_coords = plot_top + plot_h / 2 - np.clip(data, -1.0, 1.0) * (plot_h * 0.42)
                points = [coord for xy in zip(x_coords, y_coords) for coord in xy]
                if len(points) >= 4:
                    self.create_line(*points, fill="#38bdf8", width=1.5, smooth=False)
            else:
                if not np.any(data):
                    mags = np.zeros(w, dtype=np.float32)
                else:
                    window = np.hanning(len(data))
                    spec = np.fft.rfft(data * window)
                    mags = 20.0 * np.log10(np.abs(spec) + 1e-6)
                    mags -= mags.max()
                    mags = np.clip((mags + 60.0) / 60.0, 0.0, 1.0)
                    target_bins = max(32, min(w, mags.size))
                    idx = np.linspace(0, mags.size - 1, target_bins).astype(np.int32)
                    mags = mags[idx]
                bar_w = max(1, w / max(1, len(mags)))
                for i, m in enumerate(mags):
                    x0 = i * bar_w
                    y0 = plot_top + plot_h * (1.0 - float(m))
                    self.create_line(x0, plot_top + plot_h, x0, y0, fill="#22c55e")
    ...
    style.configure("StatusGood.TLabel", foreground="#166534")
    style.configure("StatusWarn.TLabel", foreground="#92400e")
    style.configure("Tiny.TLabel", font=("TkDefaultFont", 8))
    ...
    def apply_preset(kind: str) -> None:
        presets = {
            "Balanced": {"samplerate": 16000, "opus_bitrate": 8000, "opus_frame_ms": 20, "opus_complexity": 2, "opus_vbr": True, "opus_dtx": False},
            "Low bandwidth": {"samplerate": 16000, "opus_bitrate": 6000, "opus_frame_ms": 20, "opus_complexity": 1, "opus_vbr": True, "opus_dtx": True},
            "Better quality": {"samplerate": 16000, "opus_bitrate": 12000, "opus_frame_ms": 20, "opus_complexity": 4, "opus_vbr": True, "opus_dtx": False},
        }
        data = presets.get(kind)
        if not data:
            return
        for k, v in data.items():
            vars_map[k].set(v if isinstance(vars_map[k], tk.BooleanVar) else str(v))
    ...
    def set_meter(progress: ttk.Progressbar, value: float) -> None:
        progress["value"] = max(0.0, min(100.0, value * 100.0))
    ...
    def refresh_status() -> None:
        ...
        if bridge is None:
            ...
            detail_var.set("Press Start after setting peer IP, ports, and devices.")
            set_meter(input_meter, 0.0)
            set_meter(rx_meter, 0.0)
            wave_in.redraw(np.zeros(256, dtype=np.float32), 16000)
            spec_in.redraw(np.zeros(256, dtype=np.float32), 16000)
            wave_rx.redraw(np.zeros(256, dtype=np.float32), 16000)
            spec_rx.redraw(np.zeros(256, dtype=np.float32), 16000)
        else:
            status_var.set(bridge.status_text())
            conn = bridge.connection_state()
            conn_var.set(conn)
            vis = bridge.get_visual_state()
            now = time.time()
            tx_age = now - vis["last_audio_tx_at"] if vis["last_audio_tx_at"] else 999.0
            rx_age = now - vis["last_audio_rx_at"] if vis["last_audio_rx_at"] else 999.0
            detail_var.set(
                f"Voice detect: {'ON' if vis['voice'] else 'OFF'}    "
                f"TX audio: {'active' if tx_age < 1.0 else 'idle'}    "
                f"RX audio: {'active' if rx_age < 1.0 else 'idle'}"
            )
            set_meter(input_meter, vis["input_rms"] * 4.0)
            set_meter(rx_meter, vis["rx_rms"] * 4.0)
            wave_in.redraw(vis["input_wave"], vis["samplerate"])
            spec_in.redraw(vis["input_wave"][-1024:], vis["samplerate"])
            wave_rx.redraw(vis["rx_wave"], vis["samplerate"])
            spec_rx.redraw(vis["rx_wave"][-1024:], vis["samplerate"])
    ...
    start_btn = ttk.Button(toolbar, text="Start", command=start_bridge, style="Primary.TButton")
    stop_btn = ttk.Button(toolbar, text="Stop", command=stop_bridge, state="disabled")
    test_btn = ttk.Button(toolbar, text="Test", command=test_connection, state="disabled")
    devices_btn = ttk.Button(toolbar, text="Devices", command=lambda: log(list_audio_devices()))
    refresh_btn = ttk.Button(toolbar, text="Refresh", command=refresh_devices)
    load_btn = ttk.Button(toolbar, text="Load YAML", command=load_config_dialog)
    save_btn = ttk.Button(toolbar, text="Save YAML", command=save_config_dialog)
    for w in [start_btn, stop_btn, test_btn, devices_btn, refresh_btn, load_btn, save_btn]:
        w.pack(side="left", padx=(0, 6))

    preset_var = tk.StringVar(value="Balanced")
    ttk.Label(toolbar, text="Preset").pack(side="left", padx=(8, 4))
    preset_box = ttk.Combobox(toolbar, state="readonly", values=["Balanced", "Low bandwidth", "Better quality"], textvariable=preset_var, width=16)
    preset_box.pack(side="left")
    ttk.Button(toolbar, text="Apply", command=lambda: apply_preset(preset_var.get())).pack(side="left", padx=(6, 0))
    ...
    net_frame = ttk.LabelFrame(left, text="Connection", style="Card.TLabelframe")
    net_frame.grid(row=0, column=0, sticky="ew", pady=(0, 8))
    net_frame.columnconfigure(1, weight=1)

    for r, (label, key) in enumerate([("Peer host", "peer_host"), ("Peer port", "peer_port"), ("Listen port", "listen_port")]):
        ttk.Label(net_frame, text=label).grid(row=r, column=0, sticky="w", padx=(0, 6), pady=3)
        ttk.Entry(net_frame, textvariable=vars_map[key], width=16).grid(row=r, column=1, sticky="ew", pady=3)

    config_path_var = tk.StringVar(value=current_config_path["path"])
    ttk.Label(net_frame, text="Config").grid(row=3, column=0, sticky="w", padx=(0, 6), pady=3)
    ttk.Entry(net_frame, textvariable=config_path_var).grid(row=3, column=1, sticky="ew", pady=3)

    audio_frame = ttk.LabelFrame(left, text="Audio", style="Card.TLabelframe")
    audio_frame.grid(row=1, column=0, sticky="ew", pady=(0, 8))
    audio_frame.columnconfigure(1, weight=1)

    row = 0
    combos = [
        ("Samplerate", "samplerate", [8000, 12000, 16000, 24000, 48000]),
        ("Bitrate", "opus_bitrate", [4000, 6000, 8000, 10000, 12000, 16000]),
        ("Frame ms", "opus_frame_ms", [2, 5, 10, 20, 40, 60]),
        ("Complexity", "opus_complexity", list(range(0, 11))),
        ("App", "opus_application", list(OPUS_APPLICATIONS.keys())),
        ("Bandwidth", "opus_bandwidth", list(OPUS_BANDWIDTHS.keys())),
    ]
    for label, key, values in combos:
        ttk.Label(audio_frame, text=label).grid(row=row, column=0, sticky="w", padx=(0, 6), pady=3)
        ttk.Combobox(audio_frame, textvariable=vars_map[key], values=values, state="readonly", width=14).grid(row=row, column=1, sticky="ew", pady=3)
        row += 1

    ttk.Checkbutton(audio_frame, text="VBR", variable=vars_map["opus_vbr"]).grid(row=row, column=0, sticky="w", pady=(4, 0))
    ttk.Checkbutton(audio_frame, text="Opus DTX", variable=vars_map["opus_dtx"]).grid(row=row, column=1, sticky="w", pady=(4, 0))
    row += 1

    dev_frame = ttk.LabelFrame(left, text="Devices", style="Card.TLabelframe")
    dev_frame.grid(row=2, column=0, sticky="ew", pady=(0, 8))
    dev_frame.columnconfigure(1, weight=1)

    ttk.Label(dev_frame, text="Input").grid(row=0, column=0, sticky="w", padx=(0, 6), pady=3)
    input_combo = ttk.Combobox(dev_frame, textvariable=vars_map["input_device_name"])
    input_combo.grid(row=0, column=1, sticky="ew", pady=3)
    ttk.Label(dev_frame, text="Output").grid(row=1, column=0, sticky="w", padx=(0, 6), pady=3)
    output_combo = ttk.Combobox(dev_frame, textvariable=vars_map["output_device_name"])
    output_combo.grid(row=1, column=1, sticky="ew", pady=3)
    ttk.Label(dev_frame, text="Input id").grid(row=2, column=0, sticky="w", padx=(0, 6), pady=3)
    ttk.Entry(dev_frame, textvariable=vars_map["input_device"], width=10).grid(row=2, column=1, sticky="w", pady=3)
    ttk.Label(dev_frame, text="Output id").grid(row=3, column=0, sticky="w", padx=(0, 6), pady=3)
    ttk.Entry(dev_frame, textvariable=vars_map["output_device"], width=10).grid(row=3, column=1, sticky="w", pady=3)

    adv_frame = ttk.LabelFrame(left, text="Advanced", style="Card.TLabelframe")
    adv_frame.grid(row=3, column=0, sticky="ew")
    adv_frame.columnconfigure(1, weight=1)
    adv_items = [
        ("Signal", "opus_signal_type"),
        ("Packet loss %", "opus_packet_loss_perc"),
        ("VAD threshold", "vad_threshold"),
        ("Hangover", "vad_hangover_frames"),
        ("Input gain", "input_gain"),
        ("Preemphasis", "preemphasis"),
        ("Recv queue", "recv_queue_frames"),
    ]
    for r, (label, key) in enumerate(adv_items):
        ttk.Label(adv_frame, text=label).grid(row=r, column=0, sticky="w", padx=(0, 6), pady=3)
        if key == "opus_signal_type":
            ttk.Combobox(adv_frame, textvariable=vars_map[key], values=list(OPUS_SIGNAL.keys()), state="readonly", width=14).grid(row=r, column=1, sticky="ew", pady=3)
        else:
            ttk.Entry(adv_frame, textvariable=vars_map[key], width=14).grid(row=r, column=1, sticky="ew", pady=3)
    ttk.Checkbutton(adv_frame, text="Inband FEC", variable=vars_map["opus_inband_fec"]).grid(row=len(adv_items), column=0, columnspan=2, sticky="w", pady=(6, 0))
    ...
    detail_var = tk.StringVar(value="Set IP/ports, choose devices, then Start.")

    state_frame = ttk.LabelFrame(right, text="Status", style="Card.TLabelframe")
    state_frame.grid(row=0, column=0, sticky="ew", pady=(0, 8))
    state_frame.columnconfigure(1, weight=1)
    ttk.Label(state_frame, text="Link").grid(row=0, column=0, sticky="w")
    ttk.Label(state_frame, textvariable=conn_var).grid(row=0, column=1, sticky="w")
    ttk.Label(state_frame, text="Info").grid(row=1, column=0, sticky="w")
    ttk.Label(state_frame, textvariable=detail_var).grid(row=1, column=1, sticky="w")
    ttk.Label(state_frame, textvariable=status_var, style="Tiny.TLabel").grid(row=2, column=0, columnspan=2, sticky="w", pady=(4, 0))

    meter_frame = ttk.LabelFrame(right, text="Levels", style="Card.TLabelframe")
    meter_frame.grid(row=1, column=0, sticky="ew", pady=(0, 8))
    meter_frame.columnconfigure(1, weight=1)
    ttk.Label(meter_frame, text="Input").grid(row=0, column=0, sticky="w")
    input_meter = ttk.Progressbar(meter_frame, maximum=100)
    input_meter.grid(row=0, column=1, sticky="ew", padx=(6, 0), pady=3)
    ttk.Label(meter_frame, text="Receive").grid(row=1, column=0, sticky="w")
    rx_meter = ttk.Progressbar(meter_frame, maximum=100)
    rx_meter.grid(row=1, column=1, sticky="ew", padx=(6, 0), pady=3)

    graphs = ttk.Frame(right)
    graphs.grid(row=2, column=0, rowspan=2, sticky="nsew")
    ...
    wave_in = ScopeCanvas(graphs, "Mic waveform", "wave", height=150)
    spec_in = ScopeCanvas(graphs, "Mic spectrum", "spec", height=150)
    wave_rx = ScopeCanvas(graphs, "Receive waveform", "wave", height=150)
    spec_rx = ScopeCanvas(graphs, "Receive spectrum", "spec", height=150)
    wave_in.grid(row=0, column=0, sticky="nsew", padx=(0, 4), pady=(0, 4))
    spec_in.grid(row=0, column=1, sticky="nsew", padx=(4, 0), pady=(0, 4))
    wave_rx.grid(row=1, column=0, sticky="nsew", padx=(0, 4), pady=(4, 0))
    spec_rx.grid(row=1, column=1, sticky="nsew", padx=(4, 0), pady=(4, 0))

    text = scrolledtext.ScrolledText(right, height=7, state="disabled")
    text.grid(row=4, column=0, sticky="ew", pady=(8, 0))
